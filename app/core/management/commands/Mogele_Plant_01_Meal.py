...
ip_one = ZK(env('PLANT_01_MEAL_CARD'), port=4370)
# ip_one = ZK('172.16.32.81', port=4370)
conn_one = None
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        self.stdout.write("Plant 01 mealcard: Live server is starting")
        try:
            conn_one = ip_one.connect()
            print('------------Plant 01 mealcard: Device connected!!!------------')
            for attendance in conn_one.live_capture():
                if attendance is None:
                    ...
                    with open("Meal_payload_plant_01.json", 'w') as jsonfile:
                    ...
                    print("------------Plant 01 mealcard: Punch detected------------")
        except UnboundLocalError:
            print("Plant 01 mealcard: I am unable to connect to server")
        except Exception as e:
            print("Plant 01 mealcard: Process terminate : {}".format(e))
        finally:
            if conn_one:
                conn_one.disconnect()
